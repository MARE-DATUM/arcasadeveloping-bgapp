...
export function CopernicusManagement() {
  ...
  const fetchCopernicusStatus = async () => {
    try {
      ...
      const response = await fetch('https://bgapp-api-worker-dev.majearcasa.workers.dev/copernicus/angola-marine')
      
      if (response.ok) {
        const data = await response.json()
        
        // Process new official Copernicus data structure
        const hasData = data.summary?.total_products_found > 0
        const successfulApis = data.summary?.apis_successful || 0
