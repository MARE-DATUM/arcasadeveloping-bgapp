...
export function CopernicusOfficial() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Try multiple endpoints with fallback + cache bust
      const cacheBust = `?t=${Date.now()}`;
      const endpoints = [
        `https://bgapp-api-worker-dev.majearcasa.workers.dev/copernicus/angola-marine${cacheBust}`,
        `https://bgapp-copernicus-official.majearcasa.workers.dev/copernicus/angola-marine${cacheBust}`
      ];
      ...
      // If all endpoints fail, show error but don't crash
        ...
        <TabsContent value="actions" className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <Button 
              size="sm" 
              variant="outline"
              onClick={() => window.open('https://bgapp-api-worker-dev.majearcasa.workers.dev/copernicus/angola-marine', '_blank')}
            >
