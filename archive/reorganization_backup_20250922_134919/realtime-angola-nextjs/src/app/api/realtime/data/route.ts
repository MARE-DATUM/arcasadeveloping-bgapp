...
function isPointInAngolaEEZ(lat: number, lon: number): boolean {
  // Angola EEZ bounds (simplified rectangle for performance)
  const angolaEEZ = [
    [-18.02, 13.35],
    [-5.55, 12.2],
    [-5.55, 8.9],
...
function addInterpolatedPoints(data: any[]): any[] {
...
function idwInterpolation(lat: number, lon: number, points: any[], power: number = 2): number {
  let weightedSum = 0;
  let weightSum = 0;
