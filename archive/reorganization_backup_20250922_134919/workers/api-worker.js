    ...
    try {
      ...
      if (path === '/api/gfw/vessel-presence') {
        try {
          ...
          // No connectivity and no data
          console.error('GFW API: All real data endpoints failed and connectivity unavailable');
          return jsonResponse({
            vessel_count: 0,
            total_hours: 0,
            window_hours: 168,
            ...
            api_status: response ? `error_${response.status}` : 'all_endpoints_failed',
            error_type: fetchError ? fetchError.message : 'unknown',
            error_details: 'Real GFW API data not available - and no API connectivity',
            note: 'Configure proper GFW API access or check API status'
          }, 503);
